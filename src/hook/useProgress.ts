import { ProgressBarProps, SimpleProgressBarProps, TimeRange } from '@/component/ProgressBar';
import { useEffect, useMemo, useState } from 'react';
import { Optional } from '@/type/common';
import { coerceAtLeast, toMillis } from '@/util/common';
import { ChannelEpisode } from '@/type/linear';

export function useProgress({ timeRange }: ProgressBarProps): SimpleProgressBarProps {
    const [value, setValue] = useState(0);

    const max = useMemo(() => timeRange.duration ?? timeRange.end - timeRange.start, [timeRange]);
...
export function useTimeRage({ schedule }: { schedule: Optional<ChannelEpisode> }) {
