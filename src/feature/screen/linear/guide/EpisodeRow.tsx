...
export function EpisodeRow({ schedule, states, onChangeChannel, onClickEpisode }: EpisodeRowProps) {
    ...
    // const [, dispatch] = useReducerAtom(timeBarOffsetState, timeBarOffsetReducer);
    ...
    const handleClick = (schedule: ChannelEpisode) => {
        ...
        if (isAiring) {
            ...
            const index = visibleSchedule.indexOf(schedule) + COMMON_SELECTED_ROW_INDEX;
    ...
    return (
        <RowContainer>
            {hasEmptySchedule ? <EmptyEpisode onFocus={() => setSchedule(null)} /> : episodeImpl}
        </RowContainer>
    );
}
...
function EmptyEpisode({ ...rest }: { onFocus: () => void }) {
    return <EpisodeCell width={'100vw'} states={[RowState.IDLE]} title={'...'} {...rest} />;
}
