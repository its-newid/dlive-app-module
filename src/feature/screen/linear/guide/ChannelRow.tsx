...
import { ContentType, Nullable, ThumbRatio, ThumbSizes } from '../../../../type/common';
...
export function ChannelRow({ channel, states, schedule, onChangeChannel }: ChannelInfoProps) {
    ...
    const callbackFavRef = useCallback(
    ...
    return (
        <Container onFocus={handleFocus}>
            <FavContainer
                ...
                role={'button'}
                aria-labelledby={`fav ch_no-${channel.no} `}
                ref={callbackFavRef}
            >
                <MyListButton
                    id={'myList'}
                    aria-label={`accessibility_my_list channel`}
            ...
            <Thumbnail
                alt={channel?.title}
                src={channel?.thumbUrl?.[ThumbRatio.WIDE]?.[ThumbSizes.SMALL] ?? ''}
