...
export function CategoryBar() {
    ...
    const channelsAtom = useMemo(() => {
        return atom((get) => Array.from(get(totalChannelState).values()).flat());
    }, []);
    ...
    useEffect(() => {
        ...
        const focusIndex = categories.findIndex((category) => category.idx === selectedCategoryIdx);
    ...
    const changeCategory = (item: ScheduleCategory) => {
        const channel = channels.find((channel) => channel.categoryIdx === item.idx);
    ...
    const renderItem = (item: ScheduleCategory) => {
        ...
        return (
            <Category
                ...
                role={'button'}
                aria-labelledby={
                    selectedCategoryIdx === item.idx ? `list category-${item.idx}` : ''
                }
            >
