...
import { useItemNumbersInListings, useListingsTable } from '../hook/useListingsTable';
import useOverlay from '../hook/useOverlay';
import { EpisodeRow } from './EpisodeRow';
import { coerceAtMost, coerceIn } from '@/util/common';
import { ChannelRow } from './ChannelRow';
import SelectedIcon from '@/asset/icSelectedTriangle.svg?react';
...
export default function Listings() {
    ...
    const channels = useMemo(() => Array.from(channelMap.values()).flat(), [channelMap]);
    ...
    const visibleChannels = useMemo(() => {
        ...
        const endIndex = coerceAtMost(selectedChannelIndex + itemNumbers, channels.length);
        const startIndex = coerceIn(selectedChannelIndex - 2, 0, endIndex - itemNumbers);
    ...
    const handleChangeEpisode = useCallback(
        (index: number, channel: ScheduleChannel) => {
            const columnIndex = visibleChannels.list.findIndex((ch) => ch === channel);
        ...
        (channel: ScheduleChannel) => {
            ...
            for (const categoryIdx of Array.from(channelMap.keys())) {
                ...
                const category = categories.find(
                    (category) => category.idx === channel.categoryIdx
                );
                if (category) {
                    header = <RowHeader title={category.name} />;
                }
                break;
            }
            ...
            <React.Fragment key={`${channel.categoryIdx}_${channel.contentId}`}>
                ...
                {channel === EmptyMyListItem ? (
                    <EmptyMyList
                        ...
                        onClick={() => {
                            if (currentScheduleFocus !== ScheduleFocusState.LISTINGS) {
                    ...
                    <RowContents channel={channel}>
                        {(props) => (
                            <>
                                ...
                                <EpisodeRow
                                    {...props}
                                    onClickEpisode={(index) => handleChangeEpisode(index, channel)}
                                />
                            </>
                        )}
                    </RowContents>
...
function RowContents({ channel, children }: ChannelRowProps) {
    ...
    const [selectedChannel, setSelectedChannel] = useAtom(selectedChannelSelector);
    ...
    return (
        <RowContainer onFocus={() => setSelectedChannel(channel)}>
            {states.some((state) => state.includes(RowState.SELECTED)) && <SelectedSchedule />}
            {children({
                schedule,
                states,
