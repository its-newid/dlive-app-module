...
import { MiniBannerLoader as Loading } from '@/feature/screen/linear/banner/MiniBannerLoader';
    ...
    return (
        <Container aria-hidden={'true'}>
            {error ? (
                ...
                <Content>
                    <ChannelNo aria-label={`${t('accessibility_ch') + info.channelInfo?.no}`}>
                        Ch {info.channelInfo?.no}
                    </ChannelNo>
            ...
            <Thumbnail
                alt={''}
                src={info.channelInfo?.thumbUrl?.[ThumbRatio.WIDE]?.[ThumbSizes.SMALL] ?? ''}
            />
        </Container>
    );
}
...
function TimeInfo({ schedule }: { schedule: Optional<ChannelEpisode> }) {
    ...
    return state === VideoState.IDLE ? <LoadingBar /> : <Progressbar timeRange={timeRange} />;
}
