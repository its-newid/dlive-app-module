...
import { Group } from '@/component/anim/Group.tsx';
...
export function Toolbar() {
    ...
    useEffect(() => {
        function handleKeyDown(event: KeyboardEvent) {
            ...
            if (isFullDescriptionVisible && ![ENTER, ESCAPE].includes(keyCode)) {
                return;
            }

            const changeMenu = () => {
                const delta = keyCode === LEFT ? -1 : 1;
                const currentIndex = menuList.findIndex((menu) => menu === currentMenu);
                const targetIndex = coerceIn(currentIndex + delta, 0, menuList.length - 1);
    ...
    return (
        <>
            <Container>
                <ToolbarLeft>
                    {!isFullDescriptionVisible && (
                        <>
                            <GuideButton showMessage={handleMessage} menuRef={menuRef} />
            ...
            {isToastVisible && (
                <Toast isVisible={isToastVisible} role={'status'} aria-label={message}>
                    {message}
                </Toast>
            )}
        </>
...
const GuideButton = ({ menuRef, showMessage, ...rest }: GuideButtonProps) => {
    ...
    const elements: Child = {
        ...
        elements: <span key={type}>{t('live_screen_channel_banner_guide')}</span>
    };
    ...
    return <ToolButton item={elements} onClick={handleClick} ref={callbackRef} {...rest} />;
};
...
const MyListButton = ({ menuRef, ...rest }: ButtonProps) => {
    ...
    const elements: Child = {
        ...
        elements: (
            <>
                <StyledMyListButton key={`${type}.img`} isMyList={isMyListChannel} />
                <span key={`${type}.txt`}>{t('live_screen_channel_banner_my_list')}</span>
            </>
        )
    };
...
const MoreButton = ({ menuRef, ...rest }: ButtonProps) => {
    ...
    const elements: Child = isFullDescriptionVisible
        ? {
              type: ButtonType.TEXT_ONLY,
              elements: (
                  <span key={type}>{t('live_screen_channel_banner_full_description_close')}</span>
              )
          }
        : {
              type: ButtonType.BOTH,
              elements: (
                  <>
                      ...
                      <span key={type}>{t('live_screen_channel_banner_full_description')}</span>
                  </>
              )
          };
