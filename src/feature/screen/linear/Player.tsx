...
import { commonLinearHlsConfig, useHls } from '@/hook/useHls';
...
import { useMakeLinearAdsParameters } from '@/feature/screen/linear/hook/useMakeLinearAdsParameters';
import { useVideo } from '@/feature/screen/linear/hook/useVideo';
...
export default function Player({ url }: LiveVideoProps) {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                const isKnownError =
                    ...
                    error.name === AUTOPLAY_NOT_ALLOWED;
    ...
    useEffect(() => {
        ...
        if (state === VideoState.LOADED) {
            ...
            if (
                !hasErrorVideoSrc(videoRef.current) &&
                errorType !== VideoErrorState.IDLE
            ) {
    ...
    return url ? (
        <>
            ...
            {isFailedAutoplayError(errorState) && (
